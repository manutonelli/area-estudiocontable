...
// ─── Config ──────────────────────────────────────────────────────────────────
const GROQ_API_KEY = "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";
...
- Las categorías van de A a K según facturación anual y actividad (servicios vs. venta de cosas muebles/actividad primaria).
- Los topes de facturación se actualizan periódicamente por AFIP. Recomendá siempre verificar los valores vigentes en afip.gob.ar.
...
- El IVA grava las ventas de bienes y prestaciones de servicios. Alícuota general 21%, reducida 10.5%, diferencial 27%.
- Los responsables inscriptos presentan DDJJ mensual (o bimestral para pequeños).
- Ingresos Brutos es un impuesto provincial. En Buenos Aires lo recauda ARBA. Puede ser convenio multilateral si se opera en varias provincias.
...
- Los empleados en relación de dependencia pueden tener retención a través del empleador (Formulario 572).

SOCIEDADES (SRL, SA, SAS):
- SRL: 2 o más socios (o unipersonal). Capital mínimo bajo. Inscripción en IGJ o Registro Provincial.
- SA: capital mínimo más alto, directorio, asamblea de accionistas.
- SAS: más ágil y económica, se constituye digitalmente.

AFIP TRÁMITES Y VENCIMIENTOS:
- La clave fiscal de AFIP permite hacer trámites online: emitir facturas, presentar DDJJ, consultar deuda.
- Los vencimientos varían según CUIT (terminación). Verificar siempre el calendario en afip.gob.ar.
- Para inscribirse en AFIP se necesita DNI y clave fiscal nivel 3.
- Los monotributistas pagan su cuota mensualmente hasta el día 20 de cada mes.
...
export function ChatWidget() {
  ...
  async function sendMessage(text?: string) {
    ...
    try {
      ...
      const reply: string = data.choices?.[0]?.message?.content?.trim() ?? "Lo siento, hubo un error. Intentá de nuevo.";
